...
void filter1(vector<vector<vector<int>>> &vec) {
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			...
			//Saturation Filter
			int average = (r + g + b) / 3;
			int newRed = average + (int)((r - average) * saturation);
			int newGreen = average + (int)((g - average) * saturation);
			int newBlue = average + (int)((b - average) * saturation);
		   ...
		   //	Lower the brightness of every pixel
			newRed = (int)(newRed * 1.03);
			newGreen = (int)(newGreen * 1.03);
			newBlue = (int)(newBlue * 1.03);
			if (newRed < 0) newRed = 0;
			else if (newRed > 255) newRed = 255;
			if (newGreen < 0) newGreen = 0;
			else if (newGreen > 255) newGreen = 255;
			...
			//Inversion Filter
			int invR = 255 - newRed;
			int invG = 255 - newGreen;
			int invB = 255 - newBlue;
